...
class Generate
{
    ...
    public function run($data, $option = 'all')
    {
        ...
        if ($action != 'buildDir') {
            // 文件路径
            $pathView = APP_PATH . $this->module . DS .'view'.DS. $this->dir . $this->nameLower . DS;
            $pathTemplate = APP_PATH . 'admin' . DS . "template" . DS;
            $fileName = APP_PATH . "%MODULE%" . DS . "%NAME%" . DS . $this->dir . $this->name . ".php";
            $code = $this->parseCode();
            // 执行方法
    ...
    private function buildEdit($path, $pathTemplate, $fileName, $tableName, $code, $data)
    {
        ...
        return file_put_contents($file, str_replace(
            ["[MODULE]","[TABLENAME]", "[NAME]","[ROWS]","[TABLECOMMENT]", "[SET_VALUE]", "[SCRIPT]"],
            [$module,$tableName,$this->name, $code['edit'],$data['table_comment'], implode("\n", array_merge($code['set_checked'], $code['set_selected'])), implode("", $code['script_edit'])],
            $template));
    }
    ...
    private function buildIndex($path, $pathTemplate, $fileName, $tableName, $code, $data)
    {
        ...
        return file_put_contents($file, str_replace(
                ["[MODULE]", "[TABLENAME]", "[TH]", "[TD]","[TABLECOMMENT]"],
                [$module, $tableName, $th, $td,$data['table_comment']],
                $template
            )
        );
    }
    ...
    private function buildAdd($path, $pathTemplate, $fileName, $tableName, $code, $data)
    {
        ...
        return file_put_contents($file, str_replace(
                ["[MODULE]","[TABLENAME]","[ROWS]","[TABLECOMMENT]"],
                [$this->module,$tableName,  $code['add'],$data['table_comment']],
                $template
            )
        );
    }
...
//        Log::write("BUILD_SQL：\n{$sql_drop};\n{$sql_create};", Log::SQL);
    ...
    /**
     * @param $form 要处理的form字段
     */
    private function td_deal($form){
        $rule='';
        $rule .= $form['name'] == "status" ? '|get_status' : '';
        $rule .= $form['name'] == "create_time" ? '|unixTo' : '';
        $rule .= $form['name'] == "update_time" ? '|unixTo' : '';
        return $rule;
    }
            ...
            foreach ($this->data['form'] as $form) {
                        ...
                        default:
                            ...
                            if ($form['name'] == 'sort') {
                                ...
                                $td[] = '<td style="padding: 0">' . "\n"
                                    ...
                                    . tab(2) . 'class="form-control text-c order-input" data-id="{$vo.id}">'
                                    . '</td>';
                            } else {
                ...
                if (!in_array($form['name'], ['id', 'isdelete', 'create_time', 'update_time'])) {
                    ...
                    switch ($form['type']) {
                        ...
                        case "number":
                        default:
                            $editField .= tab(4) . '<input type="' . $form['type'] . '" class="form-control" '
                                . 'placeholder="' . $form['title'] . '" name="' . $form['name'] . '" '
                                . 'value="' . '{$'.$this->name.'.' . $form['name'] . ' ?? \'' . $form['default'] . '\'}' . '" '
                                . $validateForm . '>' . "\n";
    ...
    {
        return tab($tab) . '<div class="radio">' . "\n"
            . tab($tab + 1) .'<label>' .  '<input type="' . $form['type'] . '" name="' . $name . '" '
            . 'id="' . $form['name'] . '-' . $key . '" value="' . $value . '"' . $validateForm . '>' . "\n"
            . tab($tab + 1) . $title . '</label>' . "\n"
            . tab($tab) . '</div>' . "\n";
    }
